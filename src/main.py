    ...
    def setup_connection(self):
        ...
        self.channel.queue_declare(
            ...
            arguments={
                ...
                "x-queue-mode": "lazy",  # Disk-backed storage (RAM efficient, mass scan protection)
                "x-max-length": 100000,  # Max 100K messages (~220 MB disk), prevents unbounded growth
                "x-overflow": "drop-head",  # Drop oldest messages when full (real-time intel priority)
                "x-message-ttl": 3600000,  # 1 hour TTL (auto-expire stale messages)
            },
        )
    ...
    def on_message(self, ch, method, properties, body):
        ...
        try:
            ...
            try:
                ...
                logging.warning(f"Failed to ack message (channel likely closed): {ack_error}")
            ...
            if error_type == ErrorType.PERMANENT:
                ...
                try:
                    ...
                    logging.warning(f"Failed to nack message (channel likely closed): {nack_error}")
                    # Channel closed - will reconnect in outer loop
                ...
                if should_requeue:
                    ...
                    try:
                        ...
                        logging.warning(f"Failed to nack message for retry (channel likely closed): {nack_error}")
                        # Channel closed - will reconnect in outer loop
                else:
                    logging.error(
                    ...
                    try:
                        ...
                        logging.warning(f"Failed to nack message to DLQ (channel likely closed): {nack_error}")
                        # Channel closed - will reconnect in outer loop
...
def main():
    ...
    try:
        ...
        logging.info(f"Successfully initialized OpenCTI connector")
