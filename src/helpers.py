...
from constants import (ATTACK_PREFIX, BEHAVIOR_PREFIX,
                       DEFAULT_THREAT_LEVEL, ENVIRONMENT, FINGERPRINT_PREFIX,
                       TECHNIQUE_PREFIX, THREAT_LEVEL_MAP)
...
def parse_iso_datetime(iso_string: Optional[str]) -> datetime:
    ...
    try:
        ...
        logging.warning(f"Failed to parse datetime '{iso_string}': {e}. Using current time.")
        return datetime.now(timezone.utc)
...
def generate_labels(state: Dict[str, Any], intelligence: Dict[str, Any], event_type: Optional[str] = None) -> List[str]:
